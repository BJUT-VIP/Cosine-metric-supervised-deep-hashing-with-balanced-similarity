function DB = cifar(dataDir, split)

[data, labels, ~, names] = cifar_load_images(dataDir);
sets = split_cifar(labels, split);
...
DB.train.data = single(data(:, :, :, train_num));
DB.train.labels =  single(labels(train_num ));
DB.test.data = single(data(:, :, :, test_num));
DB.test.labels = single(labels(test_num));
DB.database.data = single(data(:, :, :, database_num));
...
function [data, labels, set, clNames] = cifar_load_images(dataDir)
