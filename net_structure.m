function net = net_structure (net, codelens)
    ...
    n = numel(net.layers) ;  %返回数组中元素的个数。
    for i=1:n
        if ~isempty(net.layers{i}.weights)
            net.layers{i}.weights{1} = gpuArray(net.layers{i}.weights{1}) ; %表示存储在GPU上的数据
            net.layers{i}.weights{2} = gpuArray(net.layers{i}.weights{2}) ; %表示存储在GPU上的数据
        end
    end
