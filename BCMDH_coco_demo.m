function [Loss, Loss1, Loss2, map] = BCMDH_coco_demo(codelens, eta, learningrate)
    %% setting seeds
    seed = 0;
    rng('default');
    rng(seed);
...
%     if isnan(data_train), data_train = [] ; end  %�ж�����Ԫ���Ƿ����NaN����ΪNaN���ÿգ���ֹ����ѵ������
    ...
    map = zeros(1, maxIter);
    top = 5000;
    scratch = 50; % As the fch layer, we set its learning rates for the MS-COCO datasets as 50 times that of the lower layers.
    %% training
    for iter = 1: maxIter
...
%     load('BCMDH_COCO_40_.mat')
    ...
    disp(topkmap);
    disp(ndcg);
    save(['BCMDH_COCO_' num2str(codelens) '_' num2str(eta) '_' num2str(learningrate) '_' num2str(alpha) 'bits.mat'],'U_text', 'B_test', 'testL','U_dataset', 'B_dataset', 'retrievalL','topkmap', 'precision', 'HR', 'MHam2pre','ndcg');
end